...
export function PIOPreviewGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Keep children in a stable wrapper so tab/form state is not lost when subscription status resolves. */}
      <div
        className={cn(
          !isSubscribed &&
            "cursor-pointer [&_input]:pointer-events-none [&_input]:opacity-50 [&_textarea]:pointer-events-none [&_textarea]:opacity-50 [&_select]:pointer-events-none [&_select]:opacity-50 [&_[role=combobox]]:pointer-events-none [&_[role=combobox]]:opacity-50 [&_button:not([role=tab])]:pointer-events-none [&_button:not([role=tab])]:opacity-50"
        )}
        onClick={!isSubscribed ? handleGreyClick : undefined}
      >
        {children}
      </div>
    </div>
